...
const Dashboard: React.FC = () => {
  ...
  const getAgentIcon = (role: string, size: number = 24) => {
    if (role?.includes('Support')) return <MessageSquare size={size} />;
    if (role?.includes('Assistant')) return <Mail size={size} />;
    if (role?.includes('Sales')) return <TrendingUp size={size} />;
    if (role?.includes('Receptionist') || role?.includes('Voice')) return <Headphones size={size} />;
    if (role?.includes('Social')) return <Share2 size={size} />;
    if (role?.includes('Content') || role?.includes('SEO')) return <FileText size={size} />;
    if (role?.includes('Legal')) return <Shield size={size} />;
  ...
  if (error) {
    return (
      <div style={{padding: 50, color: 'white', textAlign: 'center', background: '#0A0A0F', minHeight: '100vh'}}>
        <AlertCircle size={48} color="#E2725B" style={{marginBottom: '16px'}} />
        <h3>{error}</h3>
      ...
      >
        <div style={{display: 'flex', alignItems: 'center', gap: '16px'}}>
          <div style={{
            ...
            <Clock size={24} style={{color: '#E2725B'}} />
          </div>
          <div>
              ...
              >
                ...
                <div style={{padding: '24px'}}>
                  ...
                  <div style={{marginBottom: '20px'}}>
                    ...
                    {(emp.features || []).slice(0, 4).map((feature: string, idx: number) => (
                      <div key={idx} style={{
                        display: 'flex',
                    ...
                    {(!emp.features || emp.features.length === 0) && (
                      <div style={{color: '#666', fontSize: '14px', fontStyle: 'italic'}}>
                        Loading capabilities...
                      </div>
                    )}
                    
                    {(emp.features || []).length > 4 && (
                      <p style={{color: emp.color_theme || '#E2725B', fontSize: '13px', marginTop: '10px', cursor: 'pointer'}}>
          ...
          >
            ...
            <div style={{padding: '32px'}}>
              ...
              <h3 style={{marginBottom: '16px', color: '#fff', fontSize: '20px', display: 'flex', alignItems: 'center', gap: '10px'}}>
                <Sparkles size={20} style={{color: selectedEmployee.color_theme || '#E2725B'}} />
                All Capabilities
              </h3>
              
              <div style={{marginBottom: '32px'}}>
                {(selectedEmployee.features || []).length > 0 ? (
                  (selectedEmployee.features || []).map((feature: string, idx: number) => (
                    ...
                    >
                      <div style={{
                        ...
                        <CheckCircle size={14} style={{color: selectedEmployee.color_theme || '#E2725B'}} />
                      </div>
              ...
              <h3 style={{marginBottom: '16px', color: '#fff', fontSize: '20px', display: 'flex', alignItems: 'center', gap: '10px'}}>
                <Shield size={20} style={{color: selectedEmployee.color_theme || '#E2725B'}} />
                Configuration
              </h3>
